	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand
				.setName("help")
				.setDescription(
					"Show information for the default_ring_recipients command",
				),
		)
		.addSubcommand((subcommand) =>
			subcommand
				...
				.addIntegerOption((option) =>
					option
						...
						.addChoices({ name: "Add", value: 1 }, { name: "Remove", value: 0 })
						.setRequired(true),
				)
				.addUserOption((option) =>
					option
						.setName("user")
						.setDescription(
							"The user to add/remove as a default ring recipient",
						)
						.setRequired(true),
				)
				.addChannelOption((option) =>
					option
						.setName("channel")
						.setDescription(
							"The channel to add/remove the default ring recipient for (global if not specified)",
						)
			...
			subcommand
				...
				.addChannelOption((option) =>
					option
						.setName("channel")
						.setDescription(
							"The channel to list the default ring recipients for (global if not specified)",
						)
						.addChannelTypes(ChannelType.GuildVoice)
						.setRequired(false),
				),
			...
			subcommand
				...
				.addChannelOption((option) =>
					option
						.setName("channel")
						.setDescription(
							"The channel to clear the default ring recipients for",
						)
						.addChannelTypes(ChannelType.GuildVoice)
						.setRequired(false),
				),
		...
		.addSubcommandGroup((group) =>
			group
				.setName("auto_ring")
				.setDescription(
					"Configure automatic ringing when you join a voice channel",
				)
				.addSubcommand((subcommand) =>
					subcommand
						.setName("set")
						...
						.addChannelOption((option) =>
							option
								.setName("channel")
								.setDescription(
									"The channel to set automatic ringing for (global if not specified)",
								)
								.addChannelTypes(ChannelType.GuildVoice)
								.setRequired(false),
						),
					...
					subcommand
						...
						.addChannelOption((option) =>
							option
								.setName("channel")
								.setDescription(
									"The channel to view automatic ringing settings for (global if not specified)",
								)
								.addChannelTypes(ChannelType.GuildVoice)
								.setRequired(false),
						),
		...
		if (subcommand === "help") {
			...
			interaction.reply({
				embeds: [
					new EmbedBuilder()
						...
						.setDescription(
							`This command allows you to configure your default ring recipients, which are used when running </ring default:${commandIds.get("ring")}> or if you have automatic ringing enabled`,
						)
						.addFields(
							{ name: "help", value: "Show this help message" },
							{
								name: "edit",
								value:
									"Add/remove a user as a default ring recipient, either globally, or for a specified channel",
							},
							{
								name: "list",
								value:
									"View all default ring recipients, either globally, or for a specified channel",
							},
							{
								name: "clear",
								value:
									"Clear all default ring recipients, either globally, or for a specified channel",
							},
							{
								name: "auto_ring set",
								value:
									"Turn automatic ringing on or off, either globally, or for a specified channel. If a channel is specified, then global auto_ring is overridden for that channel",
							},
							{
								name: "auto_ring unset",
								value: "Unset automatic ringing override for a channel",
							},
							{
								name: "auto_ring get",
								value:
									"View whether automatic ringing is enabled, either globally, or for a specified channel",
							},
						),
				],
			...
			if (discordUser.setAutoRingEnabled(channel?.id, enabled)) {
				interaction
					.reply({
						content:
							`Automatic ringing when you join a voice channel is now \`${enabled ? "enabled" : "disabled"}\`.` +
							(enabled
			...
			interaction
				.reply({
					content:
						`Default ring will${autoRingEnabled ? "" : " not"} run automatically ${channel ? `for ${channel}` : "globally"} because ` +
						(channel && autoRingOverride !== undefined
							? `your override for ${channel} is set to \`${autoRingOverride ? "enabled" : "disabled"}\``
							: (channel ? `you have no override for ${channel} and ` : "") +
								`your global auto ring is \`${autoRingEnabled ? "enabled" : "disabled"}\``),
					flags: [MessageFlags.Ephemeral],
				})
				.catch(console.error);
		}
